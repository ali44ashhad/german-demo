...
// ---------- FALLBACK CONTENT (exact words from your original) ----------
const FALLBACK = {
  hero: {
  ...
  features: [
    {
      ...
      title: "Multi-Currency Forex Cards",
      description:
        "Load multiple currencies including EUR, USD, GBP on a single card for seamless transactions across Europe.",
      benefits: ["Zero Forex Markup", "Instant Reload", "ATM Access", "Online Tracking"],
    },
    {
      icon: "Send",
      title: "International Money Transfer",
      description:
        "Fast and secure remittance services with competitive exchange rates and minimal transfer fees.",
      benefits: ["Best Exchange Rates", "Quick Transfer", "Low Fees", "24/7 Support"],
    },
    {
      icon: "Shield",
      title: "Secure & Insured",
      description:
        "Your money is protected with advanced security features and insurance coverage.",
      benefits: ["Fraud Protection", "Insurance Cover", "PIN Protection", "Emergency Support"],
    },
    {
      icon: "Globe",
      title: "Global Acceptance",
      description:
        "Accepted at millions of locations worldwide including ATMs, shops, and online platforms.",
      benefits: ["Worldwide Acceptance", "Online Shopping", "Contactless Payments", "Mobile App"],
    },
  ],
  ...
  whyTitle: "Why Choose Our Forex Services?",
  whyBenefits: [
    {
      icon: "Zap",
  ...
  cta: {
    heading: "Ready to Get Your Forex Card?",
    sub: "Start your financial journey to Germany with the best forex solutions.",
    button: "Apply Now",
  },
};

export default function ForexCardRemittances() {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, threshold: 0.2 });
  const { t } = useTranslation("common");

  // Pull from i18n; stay safe if key is missing or not an object
  const raw = t("forexCardRemittances", { returnObjects: true, defaultValue: {} });

  const hero = isObject(raw.hero) ? raw.hero : FALLBACK.hero;
  const features = normalizeArray(raw.features, FALLBACK.features).map((f) => ({
    ...f,
    // resolve icon if provided as string; ignore if invalid
    _Icon: typeof f.icon === "string" && ICONS[f.icon] ? ICONS[f.icon] : CreditCard,
    benefits: normalizeArray(f.benefits, []),
  }));
  const processTitle = typeof raw.processTitle === "string" ? raw.processTitle : FALLBACK.processTitle;
  const process = normalizeArray(raw.process, FALLBACK.process);
  const whyTitle = typeof raw.whyTitle === "string" ? raw.whyTitle : FALLBACK.whyTitle;
  const whyBenefits = normalizeArray(raw.whyBenefits, FALLBACK.whyBenefits).map((b) => ({
    ...b,
    _Icon: typeof b.icon === "string" && ICONS[b.icon] ? ICONS[b.icon] : Zap,
  }));
      ...
      <section className="relative py-30 overflow-hidden">
        <div className="absolute inset-0">
          <div
            ...
            style={{
              backgroundImage:
                "url('https://images.unsplash.com/photo-1554224155-6726b3ff858f?ixlib=rb-4.0.3&w=1920&h=1080&fit=crop')",
              backgroundSize: "cover",
              backgroundPosition: "center",
            }}
          />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
            {features.map((feature, i) => {
              const Icon = feature._Icon || CreditCard;
              return (
                <motion.div
                  key={i}
                  className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300"
