...
const EMICalculator = ({ className = "" }) => {
  ...
  // i18n fallbacks (exact original wording preserved)
  const L = {
    title: t("emiCalculator.title", { defaultValue: "Quick EMI Calculator" }),
    subtitle: t("emiCalculator.subtitle", {
      defaultValue:
        "Estimate monthly EMI for education loans. Results use standard monthly-compounding EMI formula.",
    }),
    loanAmount: t("emiCalculator.loanAmount", { defaultValue: "Loan Amount (₹)" }),
    rate: t("emiCalculator.rate", { defaultValue: "Interest Rate (% p.a.)" }),
    tenure: t("emiCalculator.tenure", { defaultValue: "Tenure (years)" }),
    adjustTenure: t("emiCalculator.adjustTenure", {
      defaultValue: "Adjust Tenure:",
    }),
    monthlyEMI: t("emiCalculator.monthlyEMI", { defaultValue: "Monthly EMI" }),
    totalPaymentLabel: t("emiCalculator.totalPaymentLabel", {
      defaultValue: "Total Payment",
    }),
    totalInterestLabel: t("emiCalculator.totalInterestLabel", {
      defaultValue: "Total Interest",
    }),
    principal: t("emiCalculator.principal", { defaultValue: "Principal:" }),
    rateMonthly: t("emiCalculator.rateMonthly", { defaultValue: "Rate (monthly):" }),
    tenureMonths: t("emiCalculator.tenureMonths", { defaultValue: "Tenure:" }),
    notePrefix: t("emiCalculator.notePrefix", { defaultValue: "Note:" }),
    noteText: t("emiCalculator.noteText", {
      defaultValue:
        "For your example inputs (₹20,00,000 · 9.5% p.a. · 10 years) this calculator shows {emi} as Monthly EMI using the standard formula.",
    }),
    copySummary: t("emiCalculator.copySummary", { defaultValue: "Copy Summary" }),
  };
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 text-sm text-gray-600 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        <div>
          {L.rateMonthly} {(monthlyRate * 100).toFixed(4)}%
        </div>
        <div>
          {L.tenureMonths} {months} months
        </div>
      </div>
        ...
        <button
          ...
          onClick={() => {
            ...
            try {
              ...
              // ignore clipboard errors (e.g. non-HTTPS)
            }
          }}
        >
          {L.copySummary}
