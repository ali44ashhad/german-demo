  ...
  if (rawCards) {
    ...
    if (Array.isArray(rawCards)) {
      contactInfo = rawCards.map((c, idx) => ({
        ...
        icon: ICON_MAP[c.key] ?? (c.icon ? ICON_MAP[c.icon] : undefined) ?? DEFAULT_CONTACT_INFO[idx]?.icon ?? WhatsAppIcon
      }));
      ...
      contactInfo = Object.keys(rawCards).sort().map((k, idx) => {
        ...
        return {
          ...
          link: c.link ?? '#',
          color: c.color ?? 'from-green-600 to-sky-600',
          icon: ICON_MAP[c.key ?? k] ?? DEFAULT_CONTACT_INFO[idx]?.icon ?? WhatsAppIcon
        };
      });
    }
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
          ...
          <div className="lg:col-span-5 self-start lg:sticky lg:top-28">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-4">
              {contactInfo.map((contact, index) => {
                const Icon = contact.icon || ICON_MAP[contact.key] || WhatsAppIcon;
