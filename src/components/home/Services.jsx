...
const ICONS = {
  ...
  // fallback icons you can reference in translations
  'star': Star,
...
const Services = () => {
  ...
  if (raw) {
    if (Array.isArray(raw) && raw.length) {
      services = raw.map((s, i) => ({
        key: s.key ?? `svc_${i}`,
        icon: (s.icon && ICONS[s.icon]) ? ICONS[s.icon] : (ICONS[s.key] ?? DEFAULT_SERVICES[i]?.icon),
        title: s.title ?? s.name ?? '',
        description: s.description ?? s.desc ?? '',
        features: Array.isArray(s.features) ? s.features : (typeof s.features === 'string' ? [s.features] : DEFAULT_SERVICES[i]?.features ?? []),
        color: s.color ?? DEFAULT_SERVICES[i]?.color ?? 'from-green-600 to-sky-600',
        delay: typeof s.delay === 'number' ? s.delay : DEFAULT_SERVICES[i]?.delay ?? 0.1,
        image: s.image ?? DEFAULT_SERVICES[i]?.image ?? '',
        link: s.link ?? DEFAULT_SERVICES[i]?.link ?? '#'
      }));
    } else if (typeof raw === 'object') {
      // convert keyed object -> array sorted by key if possible
      const keys = Object.keys(raw).sort((a, b) => {
        const na = Number(a), nb = Number(b);
        if (!Number.isNaN(na) && !Number.isNaN(nb)) return na - nb;
        return String(a).localeCompare(String(b));
      });
      services = keys.map((k, i) => {
        const s = raw[k] || {};
        return {
          key: s.key ?? k,
          icon: (s.icon && ICONS[s.icon]) ? ICONS[s.icon] : (ICONS[s.key ?? k] ?? DEFAULT_SERVICES[i]?.icon),
          title: s.title ?? s.name ?? '',
          description: s.description ?? s.desc ?? '',
          features: Array.isArray(s.features) ? s.features : (typeof s.features === 'string' ? [s.features] : DEFAULT_SERVICES[i]?.features ?? []),
          color: s.color ?? DEFAULT_SERVICES[i]?.color ?? 'from-green-600 to-sky-600',
  ...
  return (
    <section ref={ref} className="relative py-20 bg-gradient-to-br from-white via-sky-50 to-green-50 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {services.map((service, index) => {
            const Icon = service.icon || DEFAULT_SERVICES[index]?.icon || CreditCard;
            return (
              <motion.div
