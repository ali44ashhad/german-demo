...
const Hero = () => {
  ...
  const hero = raw && typeof raw === "object" ? raw : DEFAULT;

  // ensure stats is an array even if translation provided object
  let stats = DEFAULT.stats;
  if (Array.isArray(hero.stats)) {
    stats = hero.stats;
  } else if (hero.stats && typeof hero.stats === "object") {
    // convert keyed object to sorted array (if keys numeric)
    const keys = Object.keys(hero.stats);
    if (keys.length) {
      keys.sort((a, b) => {
        const na = Number(a),
          nb = Number(b);
  ...
  useEffect(() => {
    if (videoRef.current) {
      // try to autoplay, but catch if browser blocks it
      videoRef.current
        .play()
        .catch((error) => {
          // don't crash — log for debug
  ...
  return (
    <section
      aria-label={hero.aria_hero_label || DEFAULT.aria_hero_label}
    ...
    >
      ...
      <div className="absolute inset-0 z-0" aria-hidden>
        <video
          ...
          playsInline
          className="absolute inset-0 w-full h-full object-cover"
          poster={hero.video_poster || DEFAULT.video_poster}
        >
          <source src={heroVideo} type="video/mp4" />
          {/* accessible fallback */}
      ...
      <div className="relative z-10 flex flex-col justify-center items-center text-center px-4 sm:px-6 md:px-10 lg:px-16 py-20 sm:py-28 lg:py-40">
        ...
        >
          <span className="bg-gradient-to-r from-green-300 to-sky-400 bg-clip-text text-transparent">
            {hero.title_line1 || DEFAULT.title_line1}
          </span>
          <br />
          <motion.span
            className="block text-lg sm:text-2xl md:text-4xl lg:text-5xl mt-2 sm:mt-4 text-blue-100"
            ...
            transition={{ duration: 0.6, delay: 0.6 }}
          >
            {hero.title_line2 || DEFAULT.title_line2}
          </motion.span>
        </motion.h1>
      ...
      <motion.div
        className="absolute bottom-6 sm:bottom-10 left-1/2 transform -translate-x-1/2 z-10"
        aria-hidden={false}
        aria-label={hero.scroll_indicator_label || "Scroll"}
        animate={{ y: [0, 10, 0] }}
        transition={{ duration: 2, repeat: Infinity }}
      >
